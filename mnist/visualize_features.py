...
parser.add_argument('--mlp-epochs', type=int, default=5, metavar='N',
...
def visualize_train_features(filename):
    train_loader = torch.utils.data.DataLoader(
        datasets.MNIST('../data', train=True, download=True,
                       transform=transforms.Compose([
                           transforms.ToTensor(),
        ...
        batch_size=train_set_size, shuffle=True, **kwargs)
    ...
    feat_cols = [ 'feature'+str(i) for i in range(output.shape[1]) ]
...
for epoch in range(1,args.epochs+1):
     ...
     visualize_train_features('unsup_features_'+str(epoch)+'epochs.png')
#
